...
def print_trading_output(result: dict) -> None:
    ...
    for ticker, decision in decisions.items():
        ...
        # Build analyst signals table (3 columns: Analyst, Signal, Confidence)
        analyst_table = []
        for agent, signals in result.get("analyst_signals", {}).items():
            ...
            row = [
                ...
                f"{Fore.YELLOW}{conf_val:.1f}%{Style.RESET_ALL}"
            ]
            analyst_table.append(row)
        if analyst_table:
            analyst_table = sort_analyst_signals(analyst_table)
        ...
        # Build trading decision table (2 columns: Parameter and Value)
        ...
        decision_table = [
            ...
            ["Confidence", f"{Fore.YELLOW}{confidence:.1f}%{Style.RESET_ALL}"]
        ]
        print(f"\n{Fore.WHITE}{Style.BRIGHT}TRADING DECISION:{Style.RESET_ALL} [{Fore.CYAN}{ticker}{Style.RESET_ALL}]")
        print(tabulate(decision_table, tablefmt="grid", colalign=("left", "right")))

        # Print reasoning
        reasoning = decision.get("reasoning", "No reasoning provided")
        print(f"\n{Fore.WHITE}{Style.BRIGHT}Reasoning:{Style.RESET_ALL} {Fore.CYAN}{reasoning}{Style.RESET_ALL}")

    # Build portfolio summary table (4 columns: Ticker, Action, Quantity, Confidence)
    portfolio_rows = []
    for ticker, decision in decisions.items():
        action = decision.get("action", "hold").upper()
        action_color = {
        ...
        # Debug: Ensure row has exactly 4 columns
        if len(row) != 4:
            print(f"Debug: Portfolio row for {ticker} has {len(row)} columns: {row}")
